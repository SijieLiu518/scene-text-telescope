...
class TextSR(base.TextBase):
    def __init__(self, config, args):
        super(TextSR, self).__init__(config, args)
        self.net_d = ImageDiscriminator()
        ...
        self.cri_gan = GANLoss(
            ...
            fake_label_val=0.0,
            loss_weight=10.0)
        self.cri_grad_penalty = GradientPenaltyLoss(
        loss_weight=10.0)
        ...
        for epoch in range(cfg.epochs):
            for j, data in (enumerate(train_loader)):
                ...
                if self.args.wgan_loss:
                    ...
                    # compute WGAN loss
                    real_d_pred = self.net_d(images_hr[:, :3, :, :])
                    l_d_real = self.cri_gan(real_d_pred, True, is_disc=True)
                    # fake
                    fake_d_pred = self.net_d(sr_img[:, :3, :, :].detach())
                    l_d_fake = self.cri_gan(fake_d_pred, False, is_disc=True)
                    l_d_total = l_d_real + l_d_fake
                    
                    if self.cri_grad_penalty:
                        l_grad_penalty = self.cri_grad_penalty(
                            self.net_d, images_hr[:, :3, :, :], sr_img[:, :3, :, :])
                ...
                loss_im = (image_loss + cp_loss) * 100
                ...
                if self.net_d:
                    # gan loss
                    fake_g_pred = self.net_d(sr_img[:, :3, :, :])
                ...
                if iters % cfg.displayInterval == 0:
                    ...
                    logging.info('[{}]\t'
                          ...
                          .format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                                  ...
                                  l_g_gan if self.args.wgan_loss else 0        
                                  ))
