...
class TSRN_PartialConv(nn.Module):
    def __init__(self, scale_factor=2, width=128, height=32, STN=False, srb_nums=5, mask=False, hidden_units=32):
        ...
        self.block1 = nn.Sequential(
            ...
            # PartialConvLayer(in_planes, 2*hidden_units, kernel_size=9, padding=4, activation="prelu"),
            nn.PReLU()
            # nn.ReLU()
        )
        ...
        setattr(self, 'block%d' % (srb_nums + 2),
                nn.Sequential(
                    ...
                    PartialConvLayer(2*hidden_units, 2*hidden_units, kernel_size=3, padding=1, activation=None),
    ...
    def forward(self, x):
        ...
        input = x[:, :3, :, :]
        # torchvision.utils.save_image(input, "outputs.png", padding=0)
        # ----------- start block1 ----------------
        # block = {'1': self.block1([input, masks])}        
        block = {'1': self.block1(input)}    
        
        # for i in range(self.srb_nums + 1):
        #     block[str(i + 2)] = getattr(self, 'block%d' % (i + 2))(block[str(i + 1)])
        # output, masks = block[str(self.srb_nums + 2)]
        
        # block[str(self.srb_nums + 3)] = getattr(self, 'block%d' % (self.srb_nums + 3)) \
        #     ([block['1'][0], block['1'][1]])
        block[str(self.srb_nums + 3)] = getattr(self, 'block%d' % (self.srb_nums + 3)) \
            (block['1'])
        # print("complete block8")
        # output, masks = torch.tanh(block[str(self.srb_nums + 3)][0]), torch.tanh(block[str(self.srb_nums + 3)][1])
        output = torch.tanh(block[str(self.srb_nums + 3)])
        
        torchvision.utils.save_image(output, "outputs.png", padding=0)
...
class UpsampleBLock(nn.Module):
    ...
    def forward(self, x):
        # x include imgs, masks
        outs, masks = self.conv(x)
        outs = self.pixel_shuffle(outs)
...
class PartialConvLayer (nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, bn=True, bias=False, activation="relu"):
        ...
        self.input_conv = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=padding, bias=bias)
