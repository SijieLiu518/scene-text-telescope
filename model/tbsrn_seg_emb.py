...
class SegEmb(nn.Module):
    ...
    def __init__(self, in_channels, hidden_units):
        ...
        self.in_channels=3
        self.conv0 = nn.Conv2d(self.in_channels, hidden_units, kernel_size=3, padding=1)
        self.conv1 = nn.Conv2d(1, hidden_units, kernel_size=3, padding=1)
        # mapping function
        self.SFT_scale_conv0 = nn.Conv2d(hidden_units, hidden_units, 1)
        self.SFT_scale_conv1 = nn.Conv2d(hidden_units, 2 * hidden_units, 1)
    ...
    def forward(self, x):
        ...
        # mask_y: H * W * 32
        f = self.SFT_scale_conv1(f * mask_y)
        # f: H * W * 64
        mask_g = self.SFT_shift_conv1(F.leaky_relu(self.SFT_shift_conv0(self.conv1(mask)), 0.1, inplace=True))
        # mask_y: H * W * 64
        return self.conv2(f + mask_g)
